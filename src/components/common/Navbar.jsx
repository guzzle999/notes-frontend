  ...
  return (
    <nav className="bg-[#F3F4F6]/50 border-[#E5E7EB] px-8 py-2 flex items-center justify-between">
      <div className="flex items-center space-x-4">
        <Link
          to={user ? "/dashboard" : "/"}
          className="md:text-2xl md:font-extrabold font-light text-black md:px-4 md:py-2 rounded-sm py-2 md:rounded-lg md:bg-[#B2F2BB] hover:bg-[#66ce74] transition"
        >
          📒 Notory
        </Link>
      </div>
      ...
      <div className="flex items-center md:space-x-4 gap-2">
        {isAuthRoute ? (
          ...
          <>
            <h3 className="text-[#111827] px-3 py-1 hidden md:block">
              {user.email}
            </h3>
            <Link
              to="/dashboard"
